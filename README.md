...
All pull requests should be based off current dev branch. The dev branch is named after its version number, the master branch is the stable branch.
...
```
npm run build  # Builds everything on Linux
npm run build-win  # Builds everything on Windows
npm run evolve  # Builds the game bundle
npm run evolve-less  # Builds the CSS file for the game on Linux
npm run evolve-less-win  # Builds the CSS file for the game on Windows
npm run wiki  # Builds the wiki bundle
npm run wiki-less  # Builds the Wiki CSS file on Linux
npm run wiki-less-win  # Builds the Wiki CSS file on Windows
npm run serve  # Launches the game server locally
npm run deploy  # Deploys the game to GitHub Pages on Linux (requires forking)
npm run deploy-win  # Deploys the game to GitHub Pages on Windows (requires forking)
```
